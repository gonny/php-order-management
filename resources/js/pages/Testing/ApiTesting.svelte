    ...
    <div class="container mx-auto p-6 space-y-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <Button variant="outline" href="/swagger">API Docs</Button>
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                                ...
                                <SelectContent>
                                    {#each apiClients ?? [] as client (client.id)}
                                        <SelectItem value={client.id}>
                                            {client.name ?? 'Unnamed Client'} ({client.key_id ?? 'No Key'})
                                        </SelectItem>
                                    {/each}
                                </SelectContent>
                            </Select>
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="endpoint" bind:value={selectedEndpoint} placeholder="/api/v1/endpoint" />
                        </div>

                        <!-- Execute Button -->
                        <Button onclick={executeTest} disabled={isLoading || !selectedApiClient} class="w-full">
                            {#if isLoading}
                                ...
                                <Select bind:value={selectedTemplate}>
                                    ...
                                    <SelectContent>
                                        {#each Object.entries(payloadTemplates ?? {}) as [key, template] (key)}
                                            <SelectItem value={key}>{template.name ?? 'Unnamed Template'}</SelectItem>
                                        {/each}
                                    </SelectContent>
                                </Select>
                                <Button variant="outline" onclick={loadTemplate} disabled={!selectedTemplate}>Load</Button>
                            </div>
                        </div>
                                    ...
                                    {#if testResponse.response}
                                        <div class="flex items-center gap-2">
                                            <Badge class={getStatusColor(testResponse.response.status_code)}>
                                                {testResponse.response.status_code ?? 'Unknown'}
                                            </Badge>
                                            <span class="text-sm text-muted-foreground">
                                                {testResponse.response.execution_time ?? 'N/A'}s
                                            </span>
                                        </div>
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <Button variant="outline" size="sm" onclick={() => copyToClipboard(testResponse.response.body ?? '')}>
                                                    <Copy class="h-3 w-3" />
                                                </Button>
                                            </div>
                                            <pre class="bg-muted p-3 rounded text-xs overflow-auto max-h-[300px]">{testResponse.response.body ??
                                    ...
                                    {#if testResponse.request}
                                        <div class="space-y-3">
                                            <div>
                                                ...
                                                <pre class="bg-muted p-3 rounded text-xs overflow-auto">{formatJson(
                    ...
                    <CardContent>
                        <div class="space-y-4">
                            {#each endpoints ?? [] as group (group.group)}
                                <div>
                                    <h4 class="font-medium mb-2">{group.group ?? 'Unnamed Group'}</h4>
                                    <div class="space-y-2">
                                        {#each group.endpoints ?? [] as endpoint (`${endpoint.method}-${endpoint.path}`)}
                                            ...
                                            >
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <code class="text-sm">{endpoint.path ?? '/api/v1/unknown'}</code>
                                                </div>
                                                <p class="text-xs text-muted-foreground mt-1">{endpoint.description ?? 'No description available'}</p>
                                            </button>
                                        {/each}
                                    </div>
                                </div>
                                ...
                                {#each testHistory ?? [] as test, index (test.timestamp + index)}
                                    <div class="flex items-center justify-between p-2 border rounded">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <code class="text-xs">{test.endpoint ?? '/api/v1/unknown'}</code>
                    ...
                    {#each Object.entries(payloadTemplates ?? {}) as [key, template] (key)}
                        <div class="border rounded-lg p-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-medium">{template.name ?? 'Unnamed Template'}</h4>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onclick={() => {
                            ...
                            <p class="text-sm text-muted-foreground mb-3">{template.description ?? 'No description available'}</p>
                            <details class="text-xs">
                                <summary class="cursor-pointer text-muted-foreground hover:text-foreground"> View Template Structure </summary>
