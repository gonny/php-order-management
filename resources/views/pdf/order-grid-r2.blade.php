...
<head>
    <meta charset="utf-8">
    <title>Order {{ $order->number }} - PDF Grid (R2)</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="pdf-container">
        <div class="header">
            Order #{{ $order->number }} - Generated on {{ now()->format('Y-m-d H:i:s') }}<br>
            Session: {{ $order->remote_session_id }}
        </div>
        ...
        <div class="footer">
            PDF generated at 300 DPI | Cell size: {{ $cellSize }}px | Order ID: {{ $order->id }} | Source: R2
        </div>
    </div>
</body>
</html>